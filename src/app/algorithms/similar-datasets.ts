...
export class SimilarDatasets {
  ...
  static sortDatasets(deepBlueService: DeepBlueService, cutoffValue: number, order: string,
                      datum: DeepBlueMiddlewareOverlapEnrichtmentResultItem[]):  Observable<any> {
    ...
    // this is because somehow there are sometimes entries in datum which are all undefined -> handled DeepBlueService.getComposedResultIterator
    // datum = datum.filter((d) =>
    //     !(d.dataset === undefined &&
    //     d.biosource === undefined &&
    //     d.epigenetic_mark === undefined));

    // We need to load the biosources and epigenetic marks from the server to use the deepBlueService.getBioSourceByName()
    const o_em = deepBlueService.listEpigeneticMarks();
    const o_bs = deepBlueService.listBioSources();

    return Observable.forkJoin([o_em, o_bs]).map(() =>  {

      const orderFunction = this.getOrderFunction(order);
      ...
      const cutoff = Statistics.percentile(datum.map((o: DeepBlueMiddlewareOverlapEnrichtmentResultItem) =>
          o.mean_rank), (cutoffValue / 100));
      const filtered_data = []
      ...
      const biosources: { [key: string]: number[] } = {};
      const ems: { [key: string]: number[] } = {};

      for (const ds of filtered_data) {
      ...
      const biosources_stats: { [key: string]: IStatsResult } = {};
      const ems_stats: { [key: string]: IStatsResult } = {};

      for (const bs in biosources) {
      ...
      return {
        'all': datum,
        'biosources': Object.keys(biosources_stats).map((biosource) => [biosource, biosources_stats[biosource]]).sort(
            (a: any, b: any) => a[1].mean - b[1].mean),
